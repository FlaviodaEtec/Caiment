...
const CONFIG = {
  ...
  videoPitch: 'Vídeo Pitch - CAIMENT.mp4',
...
function adicionarEstilosDoVideo() {
  if (selecionar('#estilos-video-pitch')) return;
 
  const estilo = document.createElement('style');
  estilo.id = 'estilos-video-pitch';
  estilo.textContent = `
    .modal-video-pitch {
      position: fixed;
      inset: 0;
    ...
    .modal-video-pitch.aberto .caixa-video-pitch {
      transform: translateY(0) scale(1);
    }
 
    .caixa-video-pitch h2 {
      margin: 0 0 1rem;
      color: #39215f;
    }
 
    .caixa-video-pitch video {
      display: block;
      width: 100%;
      max-height: 70vh;
      border-radius: 16px;
...
function abrirVideoPitch() {
  adicionarEstilosDoVideo();
 
  const modalExistente = selecionar('#modal-video-pitch');
  if (modalExistente) {
    modalExistente.classList.add('aberto');
    const videoExistente = selecionar('video', modalExistente);
    if (videoExistente) videoExistente.play().catch(function () {});
    return;
  }
 
  const modal = document.createElement('div');
  modal.id = 'modal-video-pitch';
  modal.className = 'modal-video-pitch aberto';
  modal.setAttribute('role', 'dialog');
  modal.setAttribute('aria-modal', 'true');
  modal.innerHTML = `
    <div class="caixa-video-pitch">
      <button class="fechar-video-pitch" type="button" aria-label="Fechar vídeo">&times;</button>
      <h2>Vídeo Pitch CAIMENT</h2>
      <video controls autoplay playsinline preload="metadata">
        <source src="${CONFIG.videoPitch}" type="video/mp4">
        Seu navegador não conseguiu carregar o vídeo.
      </video>
      <p class="erro-video-pitch"></p>
    </div>
  `;
 
  document.body.appendChild(modal);
 
  const video = selecionar('video', modal);
  const mensagemErro = selecionar('.erro-video-pitch', modal);
 
  video.addEventListener('error', function () {
    mensagemErro.textContent =
      `Não foi possível carregar "${CONFIG.videoPitch}". Confira o nome e a pasta do arquivo.`;
  });
 
  selecionar('.fechar-video-pitch', modal).addEventListener('click', function () {
    video.pause();
...
function configurarBotaoDoPitch() {
  const botaoPitch = selecionar('.pitch-button');
 
  if (!botaoPitch) return;
 
  botaoPitch.addEventListener('click', function (evento) {
    evento.preventDefault();
    abrirVideoPitch();
  });
}
...
document.addEventListener('keydown', function (evento) {
  ...
  const video = selecionar('#modal-video-pitch');
  const teste = selecionar('#modal-teste-avatar');
 
  if (video) {
    const elementoVideo = selecionar('video', video);
    if (elementoVideo) elementoVideo.pause();
    video.classList.remove('aberto');
  }
 
  if (teste) teste.classList.remove('aberto');
});
